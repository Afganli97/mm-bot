...
async def dashboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton("🔄 Сбросить локальные счётчики", callback_data="reset_api_usage")]]
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if address_type == "solana":
        ...
        await update.message.reply_text("❌ Формат адреса не распознан. Отправь валидный EVM или Solana адрес.")
...
async def show_multichain_evm_balances(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = [f"💰 Мультичейн EVM Балансы", f"Адрес: `{address}`"]
    ...
    for chain, conf in EVM_NETWORKS.items():
        ...
        try:
            ...
            if native_raw > 0:
                ...
                native_price = await price_service.get_price(chain, conf["weth"], web3=web3, weth_price_usd=0.0)
                ...
                if native_usd >= MIN_USD_VALUE or native_raw > 0:
                    ...
                    chain_lines.append(f"• {conf['native_symbol']}: {native_balance:.8f} ({display})")
        ...
        try:
            if moralis:
                moralis_tokens = await moralis.get_balances(session, address, chain=conf.get("moralis_chain", chain))
        ...
        try:
            if alchemy and chain == "ethereum":
                ...
                for t in alchemy_tokens:
                    ...
                    if not contract or contract in tokens:
                        continue
        ...
        try:
            if ankr and ankr.api_url:
                ...
                for asset in ankr_data.get("assets", []) if ankr_data else []:
                    ...
                    if not contract or contract == "0x0000000000000000000000000000000000000000" or contract in tokens:
                        continue

                    raw = asset.get("balanceRaw") or asset.get("raw")
                    try:
                        raw_int = int(raw)
                    except Exception:
                        raw_int = None
        ...
        for contract, token_data in list(tokens.items()):
            metadata = await metadata_service.get_evm_metadata(chain, contract, conf["rpc_url"])
            ...
            price = token_data.get("usd_val")
            if price and price > 0:
                price_usd = price / float(token_data.get("balance") or 1)
            else:
                price_usd = await price_service.get_price(chain, contract, web3=web3, weth_price_usd=0.0)
            ...
            if price_usd and balance is not None:
                usd_val = float(balance) * float(price_usd)
                chain_total += usd_val

            symbol = metadata.get("symbol") or token_data.get("symbol") or "?"
            name = metadata.get("name") or token_data.get("name") or "?"
            display = f"≈ ${usd_val:,.2f}" if usd_val is not None and usd_val > 0 else "?"

            chain_lines.append(f"• {symbol} ({name}): {balance:.8f} ({display})")
...
async def show_solana_balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not helius:
        await update.message.reply_text("❌ Helius API не настроен, не могу получить баланс Solana.")
        return
    ...
    lines = [f"💰 Баланс Solana", f"Адрес: `{address}`"]
    total_usd = 0.0
    ...
    try:
        ...
        if isinstance(native_balance, dict):
            ...
            if sol_amount > 0:
                sol_price = await price_service.get_price("solana", "So11111111111111111111111111111111111111111")
        ...
        await update.message.reply_text(f"❌ Ошибка получения Solana баланса: {e}")
        return
