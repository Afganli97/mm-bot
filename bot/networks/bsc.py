"""
BSC network adapter.
Free BscScan + public RPC fallback.
"""
...
class BscNetwork(BaseNetwork):
    ...
    async def get_block_by_timestamp(self, timestamp: int) -> int:
        if self.bscscan:
            return await self.bscscan.get_block_by_timestamp(self._session, timestamp)
        return await self.web3.get_block_by_timestamp_binary_search(self._session, timestamp)

    async def get_incoming_buys(self, address: str, start_block: int, end_block: int) -> List[Dict]:
        if self.bscscan:
            txs = await self.bscscan.get_token_transfers(
                self._session,
                address,
    ...
    async def get_outgoing_related_transfers(self, address: str, start_block: int, end_block: int) -> List[Dict]:
        ...
        if self.bscscan:
            normal = await self.bscscan.get_normal_transactions(
                self._session,
                address,
                start_block,
                end_block,
            ...
            internal = await self.bscscan.get_internal_transactions(
                self._session,
                address,
                start_block,
                end_block,
            ...
            token_txs = await self.bscscan.get_token_transfers(
                self._session,
                address,
                start_block=start_block,
                end_block=end_block,
            ...
            token_txs = await self.web3.get_token_transfers(
                self._session,
                address,
                direction="from",
                from_block=start_block,
